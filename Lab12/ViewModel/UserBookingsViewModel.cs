...
namespace GoodFood.ViewModel {
  class UserBookingsViewModel : ViewModelBase {
    ...
    private void UpdateBookings() {
      ...
      List<Booking> bookings = new List<Booking>(UnitOfWork.Bookings.Get());
      ...
      foreach(Booking booking in bookings) {
        if(booking.User_ID == MainViewModel.User.User_ID) {
          ...
          string restName = UnitOfWork.Restaurants.Get().FirstOrDefault(x => x.Rest_ID == booking.Rest_ID).Name;
    ...
    private void DeleteBooking() {
      int restId = UnitOfWork.Restaurants.Get().FirstOrDefault(x => x.Name == SelectedBooking.RestaurantName).Rest_ID;
