...
namespace GoodFood.ViewModel {
  class RestaurantPageViewModel : ViewModelBase {
    ...
    private int FreeTables(DateTime datetime) {
      List<Booking> list = new List<Booking>(UnitOfWork.Bookings.Get());
    ...
    private void RestaurantBooking() {
      ...
      if(datetime >= DateTime.Now && FreeTables(datetime) > 0 && !IsUserBook(datetime) && FillTables().Count > 0) {
        Booking book = new Booking(SelectedRestaurant.Rest_ID, SelectedUser.User_ID, datetime, Convert.ToInt32(Selected_table.Trim('№')));
        UnitOfWork.Bookings.Create(book);
    ...
    private void RestaurantRating() {
      if(Selected_rate != null && Selected_rate != "") {
        List<Rating> list = new List<Rating>(UnitOfWork.Ratings.Get());
        ...
        Rating rate = new Rating(SelectedRestaurant.Rest_ID, SelectedUser.User_ID, Convert.ToInt32(Selected_rate));
        UnitOfWork.Ratings.Create(rate);
        MessageBox.Show("Оценка добавлена!");
      }
    }
    public ICommand deleteRating => new DelegateCommand(DeleteRating);
    private void DeleteRating() {
      if(Selected_rate != null && Selected_rate != "") {
        List<Rating> list = new List<Rating>(UnitOfWork.Ratings.Get());
        foreach(Rating rating in list) {
          if(rating.User_ID == SelectedUser.User_ID && rating.Rest_ID == SelectedRestaurant.Rest_ID) {
            UnitOfWork.Ratings.Delete(rating);
